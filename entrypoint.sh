...
if u:
    if password:
        ...
        u.is_staff = True
        u.save()
        print('[entrypoint] Updated superuser ' + username)
    else:
        print('[entrypoint] Superuser ' + username + ' exists; no password change.')
else:
    if not password:
        import sys; sys.exit('[entrypoint] Set DJANGO_SUPERUSER_PASSWORD to create the superuser.')
    User.objects.create_superuser(username=username, email=email, password=password)
    print('[entrypoint] Created superuser ' + username)
"
else
